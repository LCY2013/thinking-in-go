		...
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				...
				case mvccpb.PUT: // 工作节点新增事件
					...
					w.formerMembers = append(w.formerMembers, member.String())
...
func (w *WorkerNode) WatchWorkers(ctx context.Context) {
	...
	memberName = w.localIP
	// 如果是IP:PORT类型
	if configs.Conf().Consistent.Hash.Type == constants.IpPort {
		memberName = fmt.Sprintf("%s:%d", w.localIP, configs.Conf().Serves[0].ServePort)
	}

	// 将自己加入一致性hash节点，一致性hash节点添加
	w.Consistent.Add(&HashNode{
		Name: memberName,
	})
	...
	async.GO(func() {
		// 监听协程
		// 监听/cron/worker/register/目录的后续变化
		watchChan = w.watcher.Watch(context.Background(), constants.JobWorkerRegisterDir, clientv3.WithPrefix(), clientv3.WithRev(getResp.Header.Revision))
		// 处理监听事件
		for watchResp = range watchChan {
			for _, watchEvent = range watchResp.Events {
				switch watchEvent.Type {
				case mvccpb.PUT: // 工作节点新增事件
					...
					// 用于重新分配，将不是自身的节点移除，移入新节点
					G_MGR.PushWorkerNodeChangeEvent(&entity.WorkerChangeEvent{
						WorkerName: memberName,
						ChangeType: constants.WorkerEventAdd,
					})
				...
				case mvccpb.DELETE: // 工作节点删除事件
					...
					// 用于重新分配，移除的节点数据分配到其他节点，大部分节点不用迁移，用于防止整个系统抖动
					G_MGR.PushWorkerNodeChangeEvent(&entity.WorkerChangeEvent{
						WorkerName: memberName,
						ChangeType: constants.WorkerEventDelete,
					})
