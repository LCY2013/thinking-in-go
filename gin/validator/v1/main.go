...
Behavior - 这些方法属于 MustBindWith 的具体调用。 如果发生绑定错误，则请求终止，并触发 c.AbortWithError(400, err).SetType(ErrorTypeBind)。
...
忽略验证
使用上述的 curl 命令运行上面的示例时会返回错误。
因为示例中 Password 使用了 binding:"required"。
如果 Password 使用 binding:"-"， 再次运行上面的示例就不会返回错误。
*/
