...
import (
	"github.com/LCY2013/thinking-in-go/middleware/mapstruct"
	"reflect"
	"testing"
...
func TestReflectCopier_CopyTo(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			copyFunc: func() (any, error) {
				i := InterfaceSrc(10)
				dst := mapstruct.Ptr(InterfaceDst(10))
				err := CopyTo(&i, dst)
				return dst, err
			},
		...
		},
		{
			...
			copyFunc: func() (any, error) {
				...
				err := CopyTo(&MultiPtrSrc{
					Name:    "a",
					Age:     mapstruct.Ptr[*int](mapstruct.Ptr[int](10)),
					Friends: nil,
				}, dst)
				return dst, err
			},
