...
import (
	"errors"
	"fmt"
	"github.com/LCY2013/thinking-in-go/middleware/feature/template/gen/annotation"
	"github.com/LCY2013/thinking-in-go/middleware/feature/template/gen/http"
...
func Test_gen(t *testing.T) {
	...
	// 我们使用 test 目录来测试
	err = gen(fmt.Sprintf("%s/test", pwd))
	if err != nil {
		t.Fatal(err)
	}
	testCases := []struct {
		...
		{
			name:     "order",
			genFile:  fmt.Sprintf("%s/test/my_order_service_gen.go", pwd),
			wantFile: fmt.Sprintf("%s/test/my_order_service_gen.txt", pwd),
		},
		{
			name:     "user",
			genFile:  fmt.Sprintf("%s/test/user_service_gen.go", pwd),
			wantFile: fmt.Sprintf("%s/test/user_service_gen.txt", pwd),
		},
	}
...
func Test_scanFiles(t *testing.T) {
	...
	user, err := filepath.Abs("./test/user_service.go")
	if err != nil {
		t.Fatal(err)
	}
	order, err := filepath.Abs("./test/order_service.go")
	if err != nil {
		t.Fatal(err)
	}
...
func Test_parseFiles(t *testing.T) {
	// 同样用 test 来测试
	// 在实际工作中，这种读写文件的测试，在逼不得已的时候，都放过去 test
