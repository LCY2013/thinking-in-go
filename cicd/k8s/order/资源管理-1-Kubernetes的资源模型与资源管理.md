...
通过Kubernetes 里对 Pod 进行 Eviction 的具体策略和实践方式具体论述，在实际的使用中，建议将 DaemonSet 的 Pod 都设置为 Guaranteed 的 QoS 类型。否则，一旦 DaemonSet 的 Pod 被回收，它又会立即在原宿主机上被重建出来，这就使得前面资源回收的动作，完全没有意义了。
